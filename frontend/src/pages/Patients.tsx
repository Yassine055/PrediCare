...
const Patients = () => {
  ...
  const filtered = patients
    .filter((p) => {
      ...
      // Les valeurs backend sont 'eleve', 'modere', 'faible' (sans accents)
      const niveauMap: Record<string, string> = { 'élevé': 'eleve', 'modéré': 'modere', 'faible': 'faible' };
      const matchFilter =
        filter === 'all' ||
        p.last_niveau === niveauMap[filter];
  ...
  const filters: { label: string; value: FilterType; color: string }[] = [
    ...
    { label: 'Élevé', value: 'élevé', color: 'risk-high' },
    { label: 'Modéré', value: 'modéré', color: 'risk-mid' },
    { label: 'Faible', value: 'faible', color: 'risk-low' },
  ];
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {paginated.map((patient) => (
                      <tr key={patient.id} className="hover:bg-gray-50/50 transition-colors">
                        ...
                        <td className="px-6 py-4">
                          <span className="font-semibold text-sm text-gray-900">{patient.last_score ?? '—'}</span>
                        </td>
